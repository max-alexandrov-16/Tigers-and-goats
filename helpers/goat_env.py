...
from helpers.helpers import place_goat, move_goat, move_tiger
...
class GOAT_ENV():
    ...
    def step(self,action):
        reward = 0
        not_all_goats_placed = int((self.turn + 1) / 2) < self.board.max_number_of_goats
        if  not_all_goats_placed:
            self.board = place_goat(self.board, action)
            self.turn += 1
        else:
            self.board = move_goat(self.board, action) 
            self.turn += 1 
        goats_won = self.board.check_goat_win()
        if goats_won:
