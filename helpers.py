...
def move_goat(board, agent_prediction,tiger_reward_scheme,goat_reward_scheme):
    ...
    if agent_prediction is None:
        ...
        for x, y in np.argwhere(board.state == 2):
            if create_adjacent_mask_goat(board.state, x, y).sum() == 0:
                available_goats[x, y] = 0
        if available_goats.sum() == 0:
            return board,tiger_reward_scheme["winning"]
